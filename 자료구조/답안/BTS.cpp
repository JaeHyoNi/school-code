...
void pr2(B_t m, string n, vector<string>& t, int num, int dep) {
	if (dep == num) { t.push_back(n); }
	dep++;
	if (m[n].first != ".") { pr2(m, m[n].first, t, num, dep); }
	if (m[n].second != ".") { pr2(m, m[n].second, t, num, dep); }
	dep--;
}
