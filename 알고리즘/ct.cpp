...
void square(int deep) {
	...
	do {
		...
		pass = true;
		for (i = 0; i < num; i++) {
			if (arr[num][i] > arr2[0][i]) { pass = false; error = i; exist_error = true; break; }
			if (diagonal1[i] > arr2[2][i]) { pass = false; error = deep+num-1-i                                                       ;; exist_error = true; break; }
			if (diagonal2[i] > arr2[3][i]) { pass = false; error = i-deep; exist_error = true; break; }
		}
		if (pass == true) {
			for (i; i < 2 * num - 1; i++) {
				if (diagonal1[i] > arr2[2][i]) { pass = false; error = deep + num - 1 - i;; exist_error = true; break; }
