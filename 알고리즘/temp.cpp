...
void backtrack(int x, int y, int num) {
	if (str1_end > str2_end) {
 		if (y <= str2_end) { 
			...
			t1 = str1; t2 = str2;
			for (int k = 0; k < mis_step; k++) {t1.pop_back();  t2.pop_back();}
			...
			if (temp_num - num >= temp_big) {
				...
				if (!answer.empty()) {
					if (!(answer.back().str == t1 || answer.back().str == t2)) answer.push_back({ t2, t1, temp_num - num });
					else answer.push_back({ t2, t1, temp_num - num });
				}
			}
			return;
		}
...
int main() {
	...
	for (ans& x : answer) {cout << x.str << " " << x.str2 << " " << x.num << endl;
	}
	cout << "진짜 정답은 STR1: " << answer[0].str2 << " STR2: " << answer[0].str2 << " NUM= " << answer[0].num;
	in.close();
	out.close();
}
